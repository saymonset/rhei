			...
			<table cellspacing="5" cellpadding="2" class="formato10 anchoTabla4">
				...
				<tr ng-hide="hideTipoEmpl() > 0">
					<td><label><fmt:message key="tipoEmpleado"
								bundle="${lang}" />:</label></td>
					<td><select ng-model="selectValue" name="tipoEmpleadoAngular"
						class="entrada" id="tipoEmpleadoAngular" size="1"
						ng-options="item.place for item in todos">
					</select> <input type="hidden" name="tipoEmpleado" id="tipoEmpleado"
						value="{{selectValue.id}}" /></td>
				</tr>
				...
				<tr>
					...
					<td><label for="nombreDefinitivoTransitorio"><fmt:message
								key="nombre" bundle="${lang}" /> <fmt:message
								key="reporte.pagoAndTributos" bundle="${lang}" />:</label></td>
					<td><input id="nombreDefinitivoTransitorio" type=text
						class=entrada name=nombreDefinitivoTransitorio
						value="${nombreDefinitivoTransitorio}" tabindex="" maxlength=60
						size=60 /></td>
				...
				<tr>
					...
					<td><input type="button" class="boton_color"
						...
						tabindex="" onclick="javascript: directoReporteAjax(2);" /></td>
				</tr>
