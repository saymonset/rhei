...
<body onload="actualizaReloj(); clickAdminMenu();clickBenefScolarMenu();">
					...
					<table cellspacing="0" cellpadding="0" class="anchoTabla4">
						<tr>
						 ...
						 <c:if test="${exito=='exito'}">
								<tr>
									<td colspan=1 class=avisoExito><fmt:message key="benef.exito"
											bundle="${lang}" />
								</tr>
							</c:if>
							<c:if test="${fracaso=='fracaso'}">
								<tr>
									<td colspan=1 class=aviso><fmt:message
											key="benef.fracaso" bundle="${lang}" />${co_tipo_beneficio}</td>
								</tr>
							</c:if>



						<c:if test="${tablaBeneficios == 'No hay registros'}">
							<tr>
						...
						<c:if test="${tablaBeneficios != 'No hay registros'}">
							<tr>
								<td>
									<form name=formPaginarBeneficios method=post action="">
										${tablaBeneficios} <input type=hidden name="tabla"
